...
-- Purchase Items (Individual medicines in each purchase)
CREATE TABLE public.purchase_items (
    ...
    -- Calculated fields (each references only base columns)
    gross_amount DECIMAL(12,2) GENERATED ALWAYS AS (quantity * purchase_rate) STORED,
    discount_amount DECIMAL(12,2) GENERATED ALWAYS AS ((quantity * purchase_rate) * discount_percentage / 100) STORED,
    taxable_amount DECIMAL(12,2) GENERATED ALWAYS AS ((quantity * purchase_rate) - ((quantity * purchase_rate) * discount_percentage / 100)) STORED,
    tax_amount DECIMAL(12,2) GENERATED ALWAYS AS (((quantity * purchase_rate) - ((quantity * purchase_rate) * discount_percentage / 100)) * tax_percentage / 100) STORED,
    net_amount DECIMAL(12,2) GENERATED ALWAYS AS (((quantity * purchase_rate) - ((quantity * purchase_rate) * discount_percentage / 100)) + (((quantity * purchase_rate) - ((quantity * purchase_rate) * discount_percentage / 100)) * tax_percentage / 100)) STORED,
...
CREATE TABLE public.stock_transactions (
    ...
    net_quantity INTEGER GENERATED ALWAYS AS (quantity_in - quantity_out) STORED,
    
    -- Stock Levels (before and after transaction)
    stock_before INTEGER NOT NULL,
    stock_after INTEGER GENERATED ALWAYS AS (stock_before + (quantity_in - quantity_out)) STORED,
...
-- Final comment
COMMENT ON SCHEMA public IS 'Pharmacy Inventory Management System - Complete database schema with multi-tenant support, inventory tracking, expiry management, and user authentication ready.'; 